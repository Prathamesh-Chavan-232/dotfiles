return {
	...
	config = function()
		...
		for _, server in pairs(servers) do
			...
			local has_custom_opts, server_custom_opts = pcall(require, "gmr.configs.lsp.settings." .. server)
			if has_custom_opts then
				server_opts = vim.tbl_deep_extend("force", server_opts, server_custom_opts)
			end

			local has_custom_commands, server_custom_commands = pcall(require, "gmr.configs.lsp.commands." .. server)
