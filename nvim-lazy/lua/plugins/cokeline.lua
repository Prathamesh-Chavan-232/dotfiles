    ...
    require("cokeline").setup({
      buffers = {
        filter_valid = function(buffer)
          ...
          return buffer.is_focused or buffer.is_modified or buffer.path:find(cwd, 1, true) == 1
        end,
      ...
      components = {
        {
          text = function(buffer)
            if buffer.is_focused then
              return ""
            elseif buffer.index > 1 and prev_buffer(buffer).is_focused then
              return ""
            end
            return ""
          end,
          fg = function(buffer)
            if buffer.is_focused or buffer.index > 1 and prev_buffer(buffer).is_focused then
        ...
        {
          text = function(buffer)
            return ((is_picking_focus() or is_picking_close()) and buffer.pick_letter or buffer.index) .. ":  "
          end,
          fg = function(buffer)
            return (is_picking_focus() or is_picking_close()) and get_hex("ErrorMsg", "fg")
              or (buffer.is_focused and get_hex("TabLineSel", "fg") or get_hex("TabLine", "fg"))
          end,
        },
        {
          text = function(buffer)
        ...
        {
          text = function(buffer)
            return (buffer.is_last and buffer.is_focused) and "" or ""
          end,
